...
def get_switch_details(configParams):
    ...
    for xSwt, xUser, xPasswd, xType in zip(swtList, usrList, passwdList, typeList):
        switchName = ''
        if xType.lower() != 'arista':
            x += 1
            switchName = xType + str(x)
        else:
            y += 1
            switchName = xType + str(y)

        swtDetails[switchName] = {}
        swtDetails[switchName]['name'] = switchName

        swtDetails[switchName]['user'], swtDetails[switchName]['passwd'] = xUser, xPasswd
        swtDetails[switchName]['type'] = xType
        if xType.lower() == 'arista':
            swtDetails[switchName]['ip'] = xSwt
        else:
            swtDetails[switchName]['url'] = 'http://' + xSwt + '/ins'
...
def update_ESC( url, swtName, tag, header, data, showResp=1):
    ...
    for th, td in zip(header, data):
        if th in ['data', 'data1', 'data2', 'stat']:
    ...
    if showResp:
        print '\nESC(%s) Response Code :%s'%(swtName, response)
...
def get_node_data_1(xnode):
    ...
    global netstat_data
    netstat_data[xnode] = node_data
    return


def get_node_ip_netstat_details():
    nodelist, threads = [], []
    ...
    for x_node in nodelist:
        t = threading.Thread(target=get_node_data_1, args=(x_node.strip(),))
        threads.append(t)
